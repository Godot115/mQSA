...
class PoissonModel():
    ...
    def par_t4(self, x, const):
        return x[0] * x[1] * const

    def par_t5(self, x, const):
        return x[0] * x[2] * const

    def par_t6(self, x, const):
        return x[1] * x[2] * const
    ...
    def par_deriv_vec(self, x):
        ...
        """
        const = math.exp(
            self.t0 + self.t1 * x[0] + self.t2 * x[1] + self.t3 * x[2] + self.t4 * x[0] * x[1] + self.t5 * x[0] * x[
                2] + self.t6 * x[1] * x[2])

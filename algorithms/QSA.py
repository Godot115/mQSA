...
class BAS_QSA(Optimizer):
    ...
    def update_business_1__(self, pop=None, current_epoch=None):
        ...
        for i in range(self.pop_size):
            ...
            F1 = beta * alpha * (E * np.abs(A - pop[i][self.ID_POS])) + np.random.exponential(0.5) * (
                    A - pop[i][self.ID_POS])
    ...
    def update_business_3__(self, pop, g_best):
        ...
        for i in range(self.pop_size):
            ...
            temp = g_best[self.ID_POS] + np.random.exponential(0.5, self.problem.n_dims) * (
                    pop[id1][self.ID_POS] - pop[i][self.ID_POS])
...
class mulQSA(QSA):
    def update_business_1__(self, pop=None, current_epoch=None):
        # self.problem.exchange_process(pop[0])
        # for idx in range(3):
        #     if current_epoch / self.epoch > random.gauss(0.5, 0.1):
        #         pop[idx] = self.problem.mul_process(pop[idx])
        A1, A2, A3 = pop[0][self.ID_POS], pop[1][self.ID_POS], pop[2][self.ID_POS]
        t1, t2, t3 = pop[0][self.ID_TAR][self.ID_FIT], pop[1][self.ID_TAR][self.ID_FIT], pop[2][self.ID_TAR][
            self.ID_FIT]
        q1, q2, q3 = self.calculate_queue_length__(t1, t2, t3)
    ...
    def update_business_2__(self, pop=None):
        ...
        for i in range(3):
            if np.random.random() > pr[i]:
                pop[i] = self.problem.mul_process(pop[i])
        A1, A2, A3 = pop[0][self.ID_POS], pop[1][self.ID_POS], pop[2][self.ID_POS]
        t1, t2, t3 = pop[0][self.ID_TAR][self.ID_FIT], pop[1][self.ID_TAR][self.ID_FIT], pop[2][self.ID_TAR][
            self.ID_FIT]
        q1, q2, q3 = self.calculate_queue_length__(t1, t2, t3)
